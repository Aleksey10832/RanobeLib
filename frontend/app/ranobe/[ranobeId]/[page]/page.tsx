    ...
    <div>
        ...
        <div>
          {ranobeInfo.chapters?.map(chapter => {
            return <div onClick={() => toChapter(chapter.number)} className="select-none mb-1 m-auto p-2 text-center text-1xl bg-gray-700 w-2xl rounded-sm hover:text-2xl cursor-pointer hover:transition-transform" key={chapter.id}>{chapter.name}</div>
          })}
        </div>
