...
req = requests.get(f"https://ranobes.com/chapters/{url}/page/1")
html = BeautifulSoup(req.text, "html.parser")
title = html.find('title').text.split(sep=' | ')[0].replace(':', '9121').replace('•', '').replace('?', '9120')
try:
    count_pages = html.find('div', class_='pages').find_all('a')
except:
    count_pages = []
...
try:
    ...
    for i in range(count_pages2):
        main_page = BeautifulSoup(requests.get(f"https://ranobes.com/chapters/{url}/page/{i+1}").text, "html.parser")
        chapters = main_page.findAll('div', class_='cat_line')
        try:
            for chapter in chapters:
                https = requests.get(chapter.find('a').get('href'))
                page = BeautifulSoup(https.text, "html.parser")
                name = page.find('h1', class_="title").text
                ...
                if imgs:
                    imgs_url = []
                    for o in imgs:
                        imgs_url.append(f"https://ranobes.com/{o.get('src')}")
                    count = 0
